...
# c_conv = np.loadtxt("./output/output_conv.txt")
# channel, rows, cols = feature_conv.squeeze().shape
# c_conv = c_conv.reshape(channel, rows, cols)
# conv_c_mean_error = np.mean(abs(feature_conv.squeeze().detach().numpy() - c_conv))
# print('Error C Conv: {:.10f}'.format(conv_c_mean_error))

# c_fc = np.loadtxt("./output/output_fc.txt")
